<?php
class Kinhnghiem_class {
  
  function Kinhnghiem_class() {}
...
//Views
  function num_views($name_search, $category, $shops) {
    ...
    $sql = "SELECT a.id FROM list_experience a
...
//Insert
  function insert_table($data) {
    ...
    $sql = "INSERT INTO list_experience (
			    id,name,code,status,language,create_dated)
				VALUES(NULL,'" . $data['name'] . "','" . $data['code'] . "','" . $data['status'] . "',  '" . $data['language'] . "','$created_date')";
    //echo $sql;exit;
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return false;
...
//Edit
  function get_id($id)
  {
    ...
    $sql = "Select a.*, a.id as id_tem from list_experience a
  ...
  function edit_table($id, $data){
    ...
    $sql = "Update list_experience set name ='" . $data['name'] . "', code ='" . $data['code'] . "',status ='" . $data['status'] . "' where id='$id' ";
...
//delete
  ...
  function delete_table($id)
  {
    global $db;
    $id = intval($id);
    $sql = "Delete from list_experience where id='$id'";
    $db->db_query($sql);
    return false;
  }
...
//Status
  function status_table($id, $status)
  {
    ...
    $sql = "Update list_experience set status ='$status' where id = $id $sr_shop";
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return 1;
  }
  ...
  function status_off_table($id)
  {
    ...
    $sql = "Update list_experience set status ='0' where id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
  ...
  function status_on_table($id)
  {
    ...
    $sql = "Update list_experience set status ='1' where id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
...
//thanh vien
  ...
  {
    global $db;
    $sql = "SELECT t1.* FROM list_experience t1 ORDER BY t1.id asc ";
    $res = $db->db_query($sql);
    $rows = $db->db_fetchrowset($res);
    return $rows;
  }
