<?php
class Filtertwos_class{
function Filtertwos_class(){}
//Views
function num_views($name_search,$category)
	{
		...
		$sql = "SELECT a.id FROM filter_two a, coupons_category b 
...
function views_table($page,$per_page,$order,$sc,$name_search,$category)
	{
		...
		FROM filter_two a, coupons_category b where a.id >= '1' and a.category_id = b.category_id 
...
function insert_table($data)
	{
		global $db;
		$sql = "INSERT INTO filter_two(id,name,filter_name,filter_url,category_id,status,pos,language) 
...
function get_id($id) 
	{			
		global $db;
		$sql = "Select *, id as id_tem from filter_two where id = $id"	;
		$res = $db->db_query($sql);
...
function edit_table($id,$data)
	{
		global $db;
		$sr= '';
		$sql = "Update filter_two set name ='".$data['name']."', filter_name ='".$data['filter_name']."', filter_url ='".$data['filter_url']."',
...
function update_order_table($id,$name,$filter_name,$filter_url,$pos) 
	{
		global $db;
		$id = intval($id);
		$sql = "Update filter_two set name ='$name',filter_name ='$filter_name',filter_url ='$filter_url', pos ='$pos' where id='$id'";
		$db->db_query($sql);
		return false;
	}
function update_filter_products($filter_url,$url_old,$category_old) 
	{
		global $db;
		$sql = "Update filter_products set url_two ='$filter_url' where url_two='$url_old' and category_id ='$category_old'";
		$db->db_query($sql);
		return false;
	}
//delete
function check_delete($id)
	{
		global $db;
		$sql = "Select id from filter_two where id = '$id' and status ='1'";
		$res = $db->db_query($sql);
...
function delete_table($id) 
	{
		global $db;
		$id = intval($id);
		$sql = "Delete from filter_two where id='$id' and status ='0'";
		$db->db_query($sql);
		return false;
	}
//Status
function status_table($id,$status)
	{
		global $db;
		$sql = "Update filter_two set status ='$status' where id = $id";
		$res = $db->db_query($sql);
		$db->db_freeresult($res);
		return 1;	
	}		
function status_off_table($id) 
	{
		global $db;
		$id = intval($id);
		$sql = "Update filter_two set status ='0' where id='$id'";
		$db->db_query($sql);
		return false;
	}
function status_on_table($id) 
	{
		global $db;
		$id = intval($id);
		$sql = "Update filter_two set status ='1' where id='$id'";
		$db->db_query($sql);
		return false;
	}
}
