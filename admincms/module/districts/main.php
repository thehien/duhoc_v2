...
function districts_process()
{
	...
	$cities = $Process->all_coupons_cities();
	...
	switch($main)
	{
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				$data['info']['district_name'] = $function->sql_injection(isset($_POST['district_name']) ? $_POST['district_name'] : "");
				$data['info']['cityid'] = $function->sql_injection(isset($_POST['cityid']) ? intval($_POST['cityid']) : 0);
				$data['info']['shipping'] = $function->sql_injection(isset($_POST['shipping']) ? intval($_POST['shipping']) : 0);
		...
		if($action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			if($check != null){
				foreach($check as $id){										
					$district_name = $function->sql_injection($_POST['district_name'.$id]); 
					$pos = $function->sql_injection($_POST['pos'.$id]); 
					$shipping = $function->sql_injection($_POST['shipping'.$id]); 
					$id = $function->sql_injection(intval($id));	
					$Process->update_order_table($id,$district_name,$shipping,$pos);		
				}
			}
			return $function->msg_box_status(msg_box_edit_succ,$msg_time,$_SESSION[$module]['url_views'],1);
		break;
