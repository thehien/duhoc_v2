...
function shops_process()
{
    ...
    switch ($main) {
        ...
        case "insert":
            ...
            if ($submit) {
                $data['info']['shopname'] = $function->sql_injection(isset($_POST['shopname']) ? $_POST['shopname'] : "");
        ...
        case "update_order":
            ...
            if ($check != null) {
                foreach ($check as $id) {
                    $shopname = $function->sql_injection($_POST['shopname' . $id]);
                    $pos = $function->sql_injection($_POST['pos' . $id]);
                    $id = $function->sql_injection(intval($id));
                    $Process->update_order_table($id, $shopname, $pos);
                }
            }
        ...
        case "delete":
            ...
            if ($check != null) {
                foreach ($check as $id) {
                    ...
                    $checks_product = $Process->check_coupons_products($id);
                    if ($checks > 0) {
