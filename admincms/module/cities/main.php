...
function cities_process()
{
	...
	switch($main)
	{
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				$data['info']['cityname'] = $function->sql_injection(isset($_POST['cityname']) ? $_POST['cityname'] : "");
				$data['info']['countryid'] = $function->sql_injection(isset($_POST['countryid']) ? intval($_POST['countryid']) : 0);
		...
		if($action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			if($check != null){
				foreach($check as $id){										
					$cityname = $function->sql_injection($_POST['cityname'.$id]); 
					$countryid = $function->sql_injection($_POST['countryid'.$id]); 
					...
					$Process->update_order_table($id,$cityname,$countryid,$shipping,$pos);		
				}
			}
			return $function->msg_box_status(msg_box_edit_succ,$msg_time,$_SESSION[$module]['url_views'],1);
		break;
