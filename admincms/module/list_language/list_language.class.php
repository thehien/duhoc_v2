<?php
class List_language_class {
  
  function List_language_class() {}
...
//Views
  function num_views($name_search, $category, $shops) {
    ...
    $sql = "SELECT a.id FROM list_languages a
...
//Insert
  function insert_table($data) {
    ...
    $sql = "INSERT INTO list_languages (
			    id,name,code,status,language,create_dated,img_flag)
				VALUES(NULL,'" . $data['name'] . "','" . $data['code'] . "','" . $data['status'] . "',  '" . $data['language'] . "','$created_date',  '" . $data['img_flag'] . "')";
    //echo $sql;exit;
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return false;
...
//Edit
  function get_id($id)
  {
    ...
    $sql = "Select a.*, a.id as id_tem from list_languages a
  ...
  function edit_table($id, $data){
    ...
    $sql = "Update list_languages set name ='" . $data['name'] . "', code ='" . $data['code'] . "', img_flag ='" . $data['img_flag'] . "',status ='" . $data['status'] . "' where id='$id' ";
...
//delete
  ...
  function delete_table($id)
  {
    global $db;
    $id = intval($id);
    $sql = "Delete from list_languages where id='$id'";
    $db->db_query($sql);
    return false;
  }
...
//Status
  function status_table($id, $status)
  {
    ...
    $sql = "Update list_languages set status ='$status' where id = $id $sr_shop";
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return 1;
  }
  ...
  function status_off_table($id)
  {
    ...
    $sql = "Update list_languages set status ='0' where id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
  ...
  function status_on_table($id)
  {
    ...
    $sql = "Update list_languages set status ='1' where id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
