<?php
class Translater_class {
  
  function Translater_class() {}
...
//Views
  function num_views($name_search, $category, $shops) {
    ...
    $sql = "SELECT a.news_id FROM coupons_jobseekers a, coupons_category b , coupons_shop c
  ...
  function num_views_code($code_search, $category, $shops) {
    ...
    $sql = "SELECT a.id FROM coupons_translater a
		where a.id >= '1' and a.language ='$language' $sr_search";
  ...
  function views_table($page, $per_page, $order, $sc, $name_search, $category, $shops) {
    ...
    $sql = "SELECT a.*,a.news_id as id_tem, b.category_name,b.category_url,b.news_url as news_urlcate
		FROM coupons_jobseekers a, coupons_category b
  ...
  function views_table_code($page, $per_page, $order, $sc, $code_search, $category, $shops)
  {
    ...
    $sql = "SELECT a.*,a.id as id_tem
		FROM coupons_translater a
		where a.id >= '1'
		and a.language ='$language' $sr_search $orderby Limit $page,$per_page";
    //echo $sql;
    $res = $db->db_query($sql);
...
//Insert
  function insert_table($data) {
    global $db;
    $created_date = time();
    $sql = "INSERT INTO coupons_translater(
			    id,name,longitude,latitude,email,mobile,url,content,address,status,
				language,fromlanguage,tolanguage,news_img,img_file,created_date)
				VALUES(NULL,'" . $data['name'] . "','" . $data['longitude'] . "','" . $data['latitude'] . "','" . $data['email'] . "','" . $data['mobile'] . "','" . $data['url'] . "','" . $data['content'] . "','" . $data['address'] . "','" . $data['status'] . "',	'" . $data['language'] . "',  '" . $data['fromlanguage'] . "',  '" . $data['tolanguage'] . "','" . $data['news_img'] . "','" . $data['img_file'] . "','$created_date')";
    //echo $sql;exit;
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return false;
...
//Edit
  function get_id($id)
  {
    ...
    $sql = "Select a.*, a.id as id_tem from coupons_translater a
		where a.id = $id $sr_shop";
    //echo $sql;
    $res = $db->db_query($sql);
  ...
  function edit_table($id, $data){
    ...
    $sql = "Update coupons_translater set name ='" . $data['name'] . "', news_img ='" . $data['news_img'] . "', img_file ='" . $data['img_file'] . "',
		longitude ='" . $data['longitude'] . "',latitude ='" . $data['latitude'] . "',email ='" . $data['email'] . "',mobile ='" . $data['mobile'] . "',
		url ='" . $data['url'] . "',content ='" . $data['content'] . "',address ='" . $data['address'] . "',fromlanguage ='" . $data['fromlanguage'] . "',tolanguage ='" . $data['tolanguage'] . "',status ='" . $data['status'] . "' where id='$id' ";
...
//delete
  ...
  function delete_table($id)
  {
    global $db;
    $id = intval($id);
    $sql = "Delete from coupons_jobseekers where news_id='$id' and status ='1' or status ='0'";
    $db->db_query($sql);
    return false;
  }
...
//Status
  function status_table($id, $status)
  {
    ...
    $sql = "Update coupons_jobseekers set status ='$status' where news_id = $id $sr_shop";
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return 1;
  }
  ...
  function home_table($id, $home)
  {
    ...
    $sql = "Update coupons_jobseekers set home ='$home' where news_id = $id $sr_shop";
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return 1;
  }
  ...
  function status_off_table($id)
  {
    ...
    $sql = "Update coupons_jobseekers set status ='0' where news_id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
  ...
  function status_on_table($id)
  {
    ...
    $sql = "Update coupons_jobseekers set status ='1' where news_id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
