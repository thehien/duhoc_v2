...
class Duhoc_program_cate_class
{
    function Duhoc_program_cate_class() { }
...
//Views
    function num_views($name_search, $category)
    {
        ...
        $sql = "SELECT a.news_id FROM list_duhoc_program_cate a, list_cate_study_abroad b 
    ...
    function views_table($page, $per_page, $order, $sc, $name_search, $category)
    {
		...
		FROM list_duhoc_program_cate a, list_cate_study_abroad b where a.news_id >= '1' and a.news_category = b.id 
...
//Insert
    function insert_table($data)
    {
        ...
        $created_date = time();
        $sql = "INSERT INTO list_duhoc_program_cate(news_id,news_name,news_category,news_url,news_img,news_content,description,
...
//Edit
    ...
    {
        global $db;
        $sql = "Select *, news_id as id_tem from list_duhoc_program_cate where news_id = $id";
        $res = $db->db_query($sql);
    ...
    function edit_table($id, $data)
    {
        global $db;
        $sr = '';
        $sql = "Update list_duhoc_program_cate set news_name ='" . $data['news_name'] . "', news_category ='" . $data['news_category'] . "',
    ...
    function update_order_table($id, $news_name, $pos)
    {
        global $db;
        $id = intval($id);
        $sql = "Update list_duhoc_program_cate set news_name ='$news_name', pos ='$pos' where news_id='$id'";
        $db->db_query($sql);
        return false;
    }
...
//delete
    ...
    {
        global $db;
        $sql = "Select news_id from list_duhoc_program_cate where news_id = '$id' and status ='1'";
        $res = $db->db_query($sql);
    ...
    function delete_table($id)
    {
        global $db;
        $id = intval($id);
        $sql = "Delete from list_duhoc_program_cate where news_id='$id' and status ='0'";
        $db->db_query($sql);
        return false;
    }
...
//Status
    ...
    {
        global $db;
        $sql = "Update list_duhoc_program_cate set status ='$status' where news_id = $id";
        $res = $db->db_query($sql);
        $db->db_freeresult($res);
        return 1;
    }
    ...
    function status_off_table($id)
    {
        global $db;
        $id = intval($id);
        $sql = "Update list_duhoc_program_cate set status ='0' where news_id='$id'";
        $db->db_query($sql);
        return false;
    }
    ...
    function status_on_table($id)
    {
        global $db;
        $id = intval($id);
        $sql = "Update list_duhoc_program_cate set status ='1' where news_id='$id'";
        $db->db_query($sql);
        return false;
    }
}
