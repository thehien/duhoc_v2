...
function supports_process()
{
	...
	switch($main)
	{
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				$data['info']['txt_name'] = $function->sql_injection(isset($_POST['txt_name']) ? $_POST['txt_name'] : "");
				$data['info']['txt_email'] = $function->sql_injection(isset($_POST['txt_email']) ? $_POST['txt_email'] : "");
				$data['info']['txt_tel'] = $function->sql_injection(isset($_POST['txt_tel']) ? $_POST['txt_tel'] : "");
				$data['info']['txt_address'] = $function->sql_injection(isset($_POST['txt_address']) ? $_POST['txt_address'] : "");
				$data['info']['txt_content'] = $function->FixQuotes(isset($_POST['txt_content']) ? $_POST['txt_content'] : "");
		...
		if($action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			if($check != null){
				foreach($check as $id){										
					$txt_name = $function->sql_injection($_POST['txt_name'.$id]); 
					$txt_email = $function->sql_injection($_POST['txt_email'.$id]); 
					$txt_tel = $function->sql_injection($_POST['txt_tel'.$id]); 
					$pos = $function->sql_injection($_POST['pos'.$id]); 
					$id = $function->sql_injection(intval($id));	
					$Process->update_order_table($id,$txt_name,$txt_email,$txt_tel,$pos);		
				}
			}
			return $function->msg_box_status(msg_box_edit_succ,$msg_time,$_SESSION[$module]['url_views'],1);
		break;
