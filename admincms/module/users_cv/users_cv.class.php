<?php
class Users_cv_class{
	function Users_cv_class(){
	}
function check_user($username)// kiem tra user ton tai
{
	global $db;
	$sql = "SELECT * FROM coupons_users_cv WHERE  username = '$username' and status ='1' and user_role not in ('4') ";		
	$rs = $db->db_query($sql);
...
{
	global $db;
	$sql = "SELECT * FROM coupons_users_cv WHERE password = '$password' and  username = '$username' and status	= '1'";
	$rs = $db->db_query($sql);
	$num = $db->db_numrows($rs);
	if($num == 1)
	{
...
{
	global $db;
	$sql = "SELECT * FROM coupons_users_cv WHERE username = '$username'";	
	$rs = $db->db_query($sql);
	$num = $db->db_numrows($rs);	
	return $num;
}			
function change_password($password) // thay doi pass
{
	...
	$sql = "update coupons_users_cv set password = '$password'";
	$res = $db->db_query($sql);
	if($res){ return true;}
	else return false;
}
...
function num_views($name_search)
{
	...
	else{$sr_search="and (firstname like '%".$name_search."%' or email like '%".$name_search."%' or mobile like '%".$name_search."%')";}

	if($_SESSION[URL_ADMIN]["userid"] !=1){$rs = "and userid > 1";}

	$sql = "SELECT userid FROM coupons_users_cv where 1 $sr_search $rs";	
	$res = $db->db_query($sql);					
...
function show_from_language_name($userId) {
    global $db;
    $sql = "SELECT (select name from list_languages t2 where t1.from_language=t2.id) as from_language_name FROM coupons_users_cv_language t1";
  ...
  function show_to_language_name($userId) {
    global $db;
    $sql = "SELECT (select name from list_languages t2 where t1.to_language=t2.id) as to_language_name FROM coupons_users_cv_language t1";
    $sql .= " WHERE t1.cv_id = $userId";
    $rs = $db->db_query($sql);
    $num = $db->db_numrows($rs);
...
function views_table($page,$per_page,$order,$sc,$name_search)
{
	global $db;
	$rs='';
	if($order ==""){$orderby = "ORDER BY userid desc ";}
	...
	FROM coupons_users_cv t1 where 1  $sr_search $rs $orderby Limit $page, $per_page";	
		//echo $sql;
	$res = $db->db_query($sql);					
	$rows = $db->db_fetchrowset($res);
	for ($i = 0; $i < count($rows); $i++) {

      $rows[$i]["from_language"] = $this->show_from_language_name($rows[$i]["userid"]);
      $rows[$i]["to_language"] = $this->show_to_language_name($rows[$i]["userid"]);
    }
	$db->db_freeresult($res);
	return $rows;
}
...
function insert_table($data)
{
	...
	$sql = "INSERT INTO coupons_users_cv(userid,lastname,password,firstname, ,email,mobile,user_role,status) 
	VALUES(NULL,'".$data['lastname']."','$password','".$data['firstname']."','".$data['email']."','".$data['mobile']."',
...
{			
	global $db;
	$sql = "Select *, userid as id_tem from coupons_users_cv where userid = $id";
	$res = $db->db_query($sql);
...
function edit_table($id,$data)
{
	global $db;
	$sr= '';
	if($data['password']){$sr=",password='".md5($data['password'])."'" ;}
	$sql = "Update coupons_users_cv set lastname='".$data['lastname']."' $sr, firstname ='".$data['firstname']."',
	email='".$data['email']."', mobile='".$data['mobile']."', status='".$data['status']."'
	where userid='$id'";
	$res = $db->db_query($sql);
	$db->db_freeresult($res);
	return false;
...
function update_order_table($id,$firstname,$email,$mobile) 
{
	global $db;
	$id = intval($id);
	$sql = "Update coupons_users_cv set firstname ='$firstname', email ='$email', mobile='$mobile' where userid='$id'";
	$db->db_query($sql);
	return false;
}				
//delete ok
...
{
	global $db;
	$sql = "Select userid from coupons_users_cv where userid = '$id' and status ='1'";
	$res = $db->db_query($sql);
...
function delete_table($id) 
{
	global $db;
	$id = intval($id);
	$sql = "Delete from coupons_users_cv where userid='$id' and status ='0'";
	$db->db_query($sql);
	return false;
}
//Status ok
...
{
	global $db;
	$sql = "Update coupons_users_cv set status ='$status' where userid = $id";
	$res = $db->db_query($sql);
	$db->db_freeresult($res);
	return 1;	
}
//Status ok
function active_translate_table($id,$active_translate)
{
	global $db;
	$sql = "Update coupons_users_cv set active_translate ='$active_translate' where userid = $id";
	$res = $db->db_query($sql);
	$db->db_freeresult($res);
	return 1;	
}		
function status_off_table($id) 
{
	global $db;
	$id = intval($id);
	$sql = "Update coupons_users_cv set status ='0' where userid='$id'";
	$db->db_query($sql);
	return false;
}
function status_on_table($id) 
{
	global $db;
	$id = intval($id);
	$sql = "Update coupons_users_cv set status ='1' where userid='$id'";
	$db->db_query($sql);
	return false;
}
...
{			
	global $db;
	$sql = "Select firstname from coupons_users_cv where userid = $id"	;
	$res = $db->db_query($sql);
	$rows = $db->db_fetchrow($res);
	$db->db_freeresult($res);
	return $rows;
...
function views_tree_action($userid,$status=0,$parent=0,$spacing='',$tree_arrays='',$level=0)
{
	...
	$language = LANG_AUGE_CMS;
	$sr ='';
	if($status){$sr ="and status ='$status'";}
	...
	if($db->db_numrows($res) > 0)
	{
		...
		foreach($rows as $tree)
		{
			...
			$level++;
			$db->db_freeresult($res);
			$tree_arrays = $this->views_tree_action($userid,$status=0,$tree['module_id'],$spacing.'',$tree_arrays,$level);
			$level--;
		}
	}return $tree_arrays;
}
