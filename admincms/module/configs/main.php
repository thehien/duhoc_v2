...
function configs_process()
{
	...
	switch($main)
	{
		...
		if($action_insert == '' and $action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			if($submit)
			{
				$data['info']['config_website'] = $function->sql_injection(isset($_POST['config_website']) ? $_POST['config_website'] : "");
				$data['info']['config_phone'] = $function->sql_injection(isset($_POST['config_phone']) ? $_POST['config_phone'] : "");
				$data['info']['config_hotline'] = $function->sql_injection(isset($_POST['config_hotline']) ? $_POST['config_hotline'] : "");
				$data['info']['config_fax'] = $function->sql_injection(isset($_POST['config_fax']) ? $_POST['config_fax'] : "");
				$data['info']['config_email'] = $function->sql_injection(isset($_POST['config_email']) ? $_POST['config_email'] : "");
				$data['info']['config_company'] = $function->sql_injection(isset($_POST['config_company']) ? $_POST['config_company'] : "");
				$data['info']['config_address'] = $function->sql_injection(isset($_POST['config_address']) ? $_POST['config_address'] : "");
				$data['info']['config_tilte'] = $function->sql_injection(isset($_POST['config_tilte']) ? $_POST['config_tilte'] : "");
				$data['info']['config_keywords'] = $function->sql_injection(isset($_POST['config_keywords']) ? $_POST['config_keywords'] : "");
				$data['info']['config_description'] = $function->sql_injection(isset($_POST['config_description']) ? $_POST['config_description'] : "");
				$data['info']['config_bank'] = $function->FixQuotes(isset($_POST['config_bank']) ? $_POST['config_bank'] : "");
				$data['info']['config_time'] = $function->sql_injection(isset($_POST['config_time']) ? $_POST['config_time'] : "");
				$data['info']['language'] = LANG_AUGE_CMS;
				
				if($data['info']['config_website']=="")
		...
		if($action_edit == ''){return $function->msg_box_status(msg_box_action,$msg_time,URL_ADMIN."?module=homes",2);}
			...
			if($check != null){
				foreach($check as $id){										
					$config_website = $function->sql_injection($_POST['config_website'.$id]); 
					$pos = $function->sql_injection($_POST['pos'.$id]); 
					$id = $function->sql_injection(intval($id));	
					$Process->update_order_table($id,$config_website,$pos);		
				}
			}
			return $function->msg_box_status(msg_box_edit_succ,$msg_time,$_SESSION[$module]['url_views'],1);
		break;
