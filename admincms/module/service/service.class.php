...
//Views
  function num_views($name_search, $category, $shops) {
    ...
    $sql = "SELECT a.id FROM list_service a
...
//Insert
  function insert_table($data) {
    ...
    $sql = "INSERT INTO list_service (
			    id,name,service_url,service_content,code,status,language,create_dated)
				VALUES(NULL,'" . $data['name'] . "','" . $data['service_url'] . "','" . $data['service_content'] . "','" . $data['code'] . "','" . $data['status'] . "',  '" . $data['language'] . "','$created_date')";
    //echo $sql;exit;
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return false;
...
//Edit
  function get_id($id)
  {
    ...
    $sql = "Select a.*, a.id as id_tem from list_service a
  ...
  function edit_table($id, $data){
    ...
    $sql = "Update list_service set name ='" . $data['name'] . "',service_url ='" . $data['service_url'] . "',service_content ='" . $data['service_content'] . "', code ='" . $data['code'] . "',status ='" . $data['status'] . "' where id='$id' ";
...
//delete
  ...
  function delete_table($id)
  {
    global $db;
    $id = intval($id);
    $sql = "Delete from list_service where id='$id'";
    $db->db_query($sql);
    return false;
  }
...
//Status
  function status_table($id, $status)
  {
    ...
    $sql = "Update list_service set status ='$status' where id = $id $sr_shop";
    $res = $db->db_query($sql);
    $db->db_freeresult($res);
    return 1;
  }
  ...
  function status_off_table($id)
  {
    ...
    $sql = "Update list_service set status ='0' where id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
  ...
  function status_on_table($id)
  {
    ...
    $sql = "Update list_service set status ='1' where id='$id' $sr_shop";
    $db->db_query($sql);
    return false;
  }
