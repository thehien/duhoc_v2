...
//Views
    function num_views($name_search, $category, $shops)
    {
        ...
        $sql = "SELECT a.id FROM coupons_special_category a
    ...
    function views_table($page, $per_page, $order, $sc, $name_search, $category, $shops)
    {
        ...
        $sql = "SELECT t1.*,t1.id as id_tem,(select t2.name from list_languages t2 where t1.from_language = t2.id ) as from_language_name,(select t2.name from list_languages t2 where t1.to_language = t2.id ) as to_language_name
    FROM coupons_language t1
    where t1.id >= 1
    ...
    function views_table_code($page, $per_page, $order, $sc, $code_search, $category, $shops)
    {
        ...
        $sql = "SELECT t1.*,t1.id as id_tem
		FROM coupons_special_category t1
		where t1.id >= 1
		and t1.language ='$language' $sr_search $orderby Limit $page,$per_page";
        //echo $sql;
        $res = $db->db_query($sql);
...
//Insert
    function insert_table($data)
    {
        global $db;
        $created_date = time();
        $sql = "INSERT INTO coupons_special_category (
			    id,category_name,category_url,language,category_content,category_detail,status,created_date,img, icon)
				VALUES(NULL,'" . $data['category_name'] . "','" . $data['category_url'] . "',	'" . $data['language'] . "',	'" . $data['category_content'] . "'
				,	'" . $data['category_detail'] . "'
				,  '" . $data['status'] . "','$created_date',  '" . $data['img'] . "',  '" . $data['icon'] . "')";
...
//Edit
    function get_id($id)
    {
        global $db;
        $sr_shop = '';
        $sql = "Select a.*, a.id as id_tem from coupons_special_category a
    ...
    function edit_table($id, $data)
    {
        ...
        $sql = "Update coupons_special_category set category_name ='" . $data['category_name'] . "'
        ,category_url ='" . $data['category_url'] . "',category_content ='" . $data['category_content'] . "',category_detail ='" . $data['category_detail'] . "',
         img ='" . $data['img']."' ";
        ...
        if ($data['icon']) {
         ...
         $sql .=",status ='" . $data['status'] . "' where id='$id' ";
...
//delete
    ...
    function delete_table($id)
    {
        global $db;
        $id = intval($id);
        $sql = "Delete from coupons_language where id='$id'";
        $db->db_query($sql);
        return false;
    }
...
//Status
    function status_table($id, $status)
    {
        ...
        $sql = "Update coupons_language set status ='$status' where id = $id $sr_shop";
        $res = $db->db_query($sql);
        $db->db_freeresult($res);
        return 1;
    }
    ...
    function status_off_table($id)
    {
        ...
        $sql = "Update coupons_language set status ='0' where id='$id' $sr_shop";
        $db->db_query($sql);
        return false;
    }
    ...
    function status_on_table($id)
    {
        ...
        $sql = "Update coupons_language set status ='1' where id='$id' $sr_shop";
        $db->db_query($sql);
        return false;
    }
...
//thanh vien
    function show_list_language()
    {
        global $db;
        $sql = "SELECT t1.* FROM list_languages t1 ORDER BY t1.id asc ";
        $res = $db->db_query($sql);
        $rows = $db->db_fetchrowset($res);
        return $rows;
    }
