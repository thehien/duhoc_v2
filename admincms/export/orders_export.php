...
<?php
	while($rows=$db->db_fetchrow($res))
	{
		...
		echo '<table cellpadding="0" cellspacing="0" style="border: 1px solid #CCCCCC; width:800px" >';
		echo '
			<tr>	
		...
		echo '
			<tr>	
				<td  valign="top" style="border: 1px solid #CCCCCC; "><strong>'.langcms_spten.'</strong></td>
				<td  valign="top" style="border: 1px solid #CCCCCC; "><strong>'.langcms_cartthanht.'</strong></td>
			</tr>';
		...
		while($row_pro=mysql_fetch_assoc($res_pro))
		{
			...
			<tr >
				...
				'.$row_pro["news_name"].' <br> '.langcms_cartchon.':  '.$row_pro["option_name"].'</td>
				<td valign="top" style="border: 1px solid #CCCCCC; ">
				'.number_format(round($row_pro["price_new"],-1)).'  *  '.$row_pro["quantity"].' 
				= '.number_format(round($row_pro["price_sum"],-1)).' VNĐ </td>
			</tr>
			';
		}
		echo '</table>';
